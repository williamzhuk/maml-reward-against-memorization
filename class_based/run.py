...
def meta_train_fn(model, exp_string, data_generator: DataGenerator,
                  ...
                  log=True, logdir='/tmp/data', k_shot=1, num_inner_updates=1, meta_lr=0.001, lam=0.):
    SUMMARY_INTERVAL = 10
    SAVE_INTERVAL = 100
    PRINT_INTERVAL = 10
...
def run_maml(n_way=5, k_shot=1, meta_batch_size=25, meta_lr=0.001,
             ...
             meta_train_iterations=10000, meta_train_k_shot=-1, meta_train_inner_update_lr=-1, lam=0.):
...
if __name__ == '__main__':
    ...
    for lam in [.1, .5, 1]:
        ...
        plt.legend()
        plt.savefig(
            'maml-nf=%d,nt=%d,lam=%f.png' % (num_filters, num_train_chars, lam),
            format='png', dpi=600)
